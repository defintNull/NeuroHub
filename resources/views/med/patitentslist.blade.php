            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @foreach ($patients as $patient)
                    <div class="p-6 text-gray-900">
                        ...
                        <h1>Surname: {{ $patient->surname }}</h1>
                        <h1>Telephone: {{ $patient->telephone }}</h1>
                        <h1>Birthdate: {{ $patient->birthdate }}</h1>
                        <button class="h-10 px-5 m-2 text-blue-100 transition-colors duration-150 bg-blue-600 rounded-lg focus:shadow-outline hover:bg-blue-700">
                            <a href="{{ route('med.patients.edit', ['patient'=>$patient->id]) }}">Edit</a>
                        </button>
                        <form method="post" action="{{ route('med.patients.destroy', $patient->id) }}">
                            @csrf
                            @method('delete')
