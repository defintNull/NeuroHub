                ...
                <ul id="radiolist" class="w-48 md:w-1/2 text-sm font-medium text-gray-900 border rounded-lg bg-blue-100 border-gray-400">
                    @if (isset($fields))
                        @for ($i=0; $i<$fields->count(); $i++)
                            <li class="w-full border-b rounded-t-lg border-gray-400">
                                <div class="flex items-center ps-3">
                                    ...
                                    <input type="text" value="{{ $fields[$i] }}" id="radio-input-{{ $i }}" name="radioinput{{ $i }}" class="w-full my-3 ml-6 mr-5 md:mr-10 ms-2 text-sm font-medium text-gray-900 bg-blue-100 focus:bg-white"/>
                                </div>
